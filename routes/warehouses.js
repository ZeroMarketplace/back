...
const warehousesCollection = db.getDB().collection('warehouses');
...
router.get(
    '/',
    function (req, res) {
        warehousesCollection.find().toArray().then((result) => {
            res.json(result);
        });
    }
);
...
router.put(
    ...
    async function (req, res, next) {
        ...
        // check exists
        warehousesCollection.findOne({_id: _id}).then(findResult => {
            if (findResult) {
                // update
                warehousesCollection.updateOne(
                    {_id: _id},
                    {
                        $set: {
                            title     : req.body.title,
...
router.delete(
    ...
    async function (req, res, next) {
        ...
        warehousesCollection.findOne({_id: _id}).then(findResult => {
            if (findResult) {
                // delete
                warehousesCollection.deleteOne({_id: _id}).then((result) => {
                    return res.sendStatus(result.acknowledged ? 200 : 400);
                });
            } else {
                return res.sendStatus(404);
